...
for i in range(2, len(sys.argv)):
    ...
    sim_name = sys.argv[i].strip("/").split("/")[-1]
...
# determine bounds for histogram ranges
...
bound_time_max = np.quantile(bounds["time"]["max"], 0.5)
...
ang_margin = 1
bound_angx_min = bound_angx_mean - ang_margin
bound_angx_max = bound_angx_mean + ang_margin
bound_angy_min = bound_angy_mean - ang_margin
bound_angy_max = bound_angy_mean + ang_margin
...
for normalize in [False, True]:
    for sim in range(len(input_paths)):
        ...
        bins[sim]["first_int"] = np.linspace(bound_first_int_min, bound_first_int_max, n_bins+1)
        hists[sim]["first_int"] = np.zeros(n_bins)
        ...
        with eventio.IACTFile(input_paths[sim] + "/" + input_name) as f:
            ...
            for telescope in range(n_telescopes):
                ...
                bins[sim][telescope]["x"] = np.linspace(-2 * radius, 2 * radius, n_bins+1)
                bins[sim][telescope]["y"] = np.linspace(-2 * radius, 2 * radius, n_bins+1)
                bins[sim][telescope]["r"] = np.linspace(0, 2 * radius, n_bins+1)
    ...
    print(f"Making plots in '{plot_path}'")
    ...
    # Parse zenith angle from the sim name
    name_split = sim_name.split("_")
    for x in name_split:
        if "z" in x:
            zen = int(x.strip("z"))
    ...
    for plot in range(len(titles)):
        for telescope in range(n_telescopes):
            ...
            # plot axis ranges
            if (zen == 60):
                if (cols[plot] in ["first_int", "zem", "photons", "angx", "angy"]):
                    ax2.set_ylim(0.05, 1.95)
                if (cols[plot] in ["x", "y", "r", "time"]):
