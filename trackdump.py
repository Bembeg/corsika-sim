...
# define binning
len_min = df_exp[df_exp["len"]>0].min()["len"]
len_max = df_exp.max()["len"]
bins = np.logspace(np.log10(len_min), np.log10(len_max), 64)
...
ax.grid(ls="dashed", c="0.85")    
# save fig
fig.savefig("plots/trackdump/dump_expon.png", dpi=dpi_val)
# clear fig
plt.clf()
...
# define binning
len_min = df_int[df_int["len"]>0].min()["len"]
len_max = df_int.max()["len"]
bins = np.logspace(np.log10(len_min), np.log10(len_max), 64)
...
ax.grid(ls="dashed", c="0.85")    
# save fig
fig.savefig("plots/trackdump/dump_interp.png", dpi=dpi_val)
# clear fig
plt.clf()